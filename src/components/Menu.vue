<template>
  <div class="relative flex w-screen" ref="menuRef">
    <div
      class="absolute w-screen h-auto flex shadow-lg justify-between items-center px-20"
    >
      <!-- Logo de la marca -->
      ...
      <div class="flex flex-row-reverse w-screen">
        ...
        <!-- Botón del menú tipo "burger" 
        <button
            id="burger-menu-button"
            type="button"
            class="inline-flex items-center gap-x-1 text-sm/6 font-semibold text-secondary"
            aria-expanded="false"
            @click="toggleMenu"
          >
            <svg
              :class="{'rotate-45': isMenuOpen, 'rotate-0': !isMenuOpen}"
              class="size-6 text-secondary transition-transform duration-300"
              viewBox="0 0 24 24"
              fill="none"
              stroke="currentColor"
              stroke-width="3"
              ...
              aria-hidden="true"
            >
              <line x1="3" y1="12" x2="21" y2="12" :class="{'opacity-0': isMenuOpen, 'opacity-100': !isMenuOpen}"></line>
              <line x1="3" y1="6" x2="21" y2="6" :class="{'rotate-45 translate-y-6 opacity-100': isMenuOpen, 'rotate-0 opacity-100': !isMenuOpen}"></line>
              <line x1="3" y1="18" x2="21" y2="18" :class="{'-rotate-45 -translate-y-6 opacity-100': isMenuOpen, 'rotate-0 opacity-100': !isMenuOpen}"></line>
            </svg>
          </button>-->
      </div>
    </div>
        ...
        >
          ...
          <div class="grid grid-cols-2 divide-x divide-gray-900/5 bg-gray-50">
            <a
              ...
              :key="index"
              :href="cta.link"
              class="flex items-center justify-center gap-x-2.5 p-3 font-semibold text-secondary hover:bg-gray-100"
            >
              <svg
                :class="cta.iconClass"
                viewBox="0 0 20 20"
...
const menuItems = [
  {
    ...
    link: "#",
    iconClass: "size-6 text-secondary group-hover:text-secondary-dark",
    iconPath:
